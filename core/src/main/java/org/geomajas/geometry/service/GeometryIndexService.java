...
public class GeometryIndexService {
	...
	/**
	 * Given a certain geometry, get the (sub)geometry, edge or vertex the index points to. This only works if the index
	 * actually points to a sub-geometry, edge or vertex.
	 * 
	 * @param geometry The geometry to search in.
	 * @param index The index that points to a sub-geometry within the given geometry or null for the complete geometry
	 * @return Returns the sub-geometry as itself, the edge as a linestring or the vertex as a point (only if they
	 *         exist)
	 * @throws GeometryIndexNotFoundException Thrown if the sub-geometry, edge or vertex could not be found within the
	 *         given geometry.
	 */
	public Geometry getGeometry(Geometry geometry, GeometryIndex index) throws GeometryIndexNotFoundException {
		...
		if (index.getType() == GeometryIndexType.TYPE_GEOMETRY && geometry.getGeometries() != null
			...
			return geometry.getGeometries()[index.getValue()];
				...
				&& (geometry.getCoordinates().length - 1) > index.getValue()) {
			Geometry edge = new Geometry(Geometry.LINE_STRING, geometry.getSrid(), geometry.getPrecision());
			edge.setCoordinates(new Coordinate[] { geometry.getCoordinates()[index.getValue()],
			...
			point.setCoordinates(new Coordinate[] { geometry.getCoordinates()[index.getValue()] });
			return point;
		}
		throw new GeometryIndexNotFoundException("Could not match index with given geometry");
	}
	...
	/**
	 * Do minimal check on whether this index could be a shell index.
	 * 
	 * @param index
	 *            The index to check.
	 * @return true or false.
